...
ALLOWED_ROOT_DIRS = {".axiom", ".github", "bulk", "data", "programs", "tests", "bo"}
ALLOWED_ROOT_FILES = {
    ".gitignore",
    "CLAUDE.md",
    "README.md",
...
def test_only_bolivia_namespace_present() -> None:
    """Bolivia scope: the only jurisdiction directory is bo/."""
    names = {d.name for d in jurisdiction_dirs()}
    assert names <= {"bo"}, f"unexpected jurisdiction dirs: {names - {'bo'}}"


def test_bo_content_buckets_exist() -> None:
    for marker in ("statutes", "regulations", "policies"):
        assert (ROOT / "bo" / marker).is_dir(), f"missing bo/{marker}"
...
def test_oracle_index_is_bo_scoped() -> None:
    payload = json.loads((ROOT / "data/oracles/oracle-index.json").read_text())
    assert payload["jurisdiction"] == "bo"


def test_source_map_is_bo_scoped() -> None:
    payload = json.loads(
...
def test_toolchain_pins_are_full_shas() -> None:
    ...
    assert re.fullmatch(
        r"[a-z]{2}-rulespec-\d{4}-\d{2}-\d{2}(?:-r\d+)?", toolchain["axiom_corpus_release"]
